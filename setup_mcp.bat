...
REM --- Step 4: .vscode/mcp.json + .cursor/mcp.json (unified with PowerShell) ---
echo [4/4] Writing MCP config (.vscode + Cursor)...
powershell -NoProfile -ExecutionPolicy Bypass -File "%PLUGIN_DIR%ensure_mcp_servers.ps1" -BootstrapIfMissing:$false
if %ERRORLEVEL% neq 0 (
    echo   ERROR: ensure_mcp_servers.ps1 failed.
    pause
    exit /b 1
)
echo.
...
echo  Configured: ue-editor-mcp + ue-editor-mcp-logs + ue-editor-mcp-insights
echo ============================================
