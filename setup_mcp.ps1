...
# --- Step 4: Generate .vscode/mcp.json + .cursor/mcp.json (3 servers each) ---
Write-Host "[4/4] Writing MCP config (.vscode + Cursor)..." -ForegroundColor Yellow

$ensureScript = Join-Path $PluginDir "ensure_mcp_servers.ps1"
if (-not (Test-Path $ensureScript)) {
    throw "Missing $ensureScript"
}
& $ensureScript -BootstrapIfMissing:$false
Write-Host ""
...
Write-Host "   2. Open VS Code or Cursor — the three MCP servers will auto-start from .vscode/mcp.json or .cursor/mcp.json"
Write-Host "   3. Use Copilot Chat to control Blueprints and analyze logs/traces"
Write-Host ""
