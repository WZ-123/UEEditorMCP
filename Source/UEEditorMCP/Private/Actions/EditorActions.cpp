...
// ============================================================================
// FGetSelectedActorsAction
// ============================================================================

TSharedPtr<FJsonObject> FGetSelectedActorsAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	if (!GEditor)
	{
	...
	for (FSelectionIterator It(GEditor->GetSelectedActorIterator()); It; ++It)
	{
		...
		SelectedActorsArray.Add(FMCPCommonUtils::ActorToJsonValue(Actor));
		SelectedCount++;
	}

	if (SelectedActorsArray.IsEmpty())
	{
		return CreateErrorResponse(TEXT("No actors are currently selected in the level."), TEXT("no_selection"));
	}

	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
	Result->SetNumberField(TEXT("count"), SelectedCount);
	Result->SetArrayField(TEXT("actors"), SelectedActorsArray);
	return CreateSuccessResponse(Result);
}
	...
	else
	{
		...
		for (UPackage* Package : DirtyPackages)
		{
			...
			if (FPackageName::TryConvertLongPackageNameToFilename(PackageName, PackageFilename, Extension))
			{
				...
				// Always attempt to find the asset regardless of package type.
				// Providing AssetToSave explicitly avoids internal fallback edge cases
				// in UPackage::SavePackage that may trigger __debugbreak() assertions
				// on certain UE 5.5 configurations when assets are in an invalid state.
				UObject* AssetToSave = Package->FindAssetInPackage();
				if (!AssetToSave)
				{
					UE_LOG(LogMCP, Warning, TEXT("UEEditorMCP SaveAll: No asset found in package '%s' (isMap=%d), attempting save with nullptr"), *PackageName, bIsMap ? 1 : 0);
				}

				UE_LOG(LogMCP, Log, TEXT("UEEditorMCP SaveAll: Saving package '%s' -> '%s' (asset=%s)"),
					*PackageName, *PackageFilename,
					AssetToSave ? *AssetToSave->GetName() : TEXT("nullptr"));
...
// ----------------------------------------------------------------------------
// FGetStreamingLevelsAction
// ----------------------------------------------------------------------------

TSharedPtr<FJsonObject> FGetStreamingLevelsAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	UWorld* World = GEditor ? GEditor->GetEditorWorldContext().World() : nullptr;
	if (!World)
	{
	...
	for (ULevelStreaming* LS : World->GetStreamingLevels())
	{
		...
		LevelObj->SetStringField(TEXT("package_name"), LS->GetWorldAssetPackageName());
		LevelObj->SetStringField(TEXT("class"), LS->GetClass()->GetName());
		LevelObj->SetBoolField(TEXT("should_be_loaded"), LS->ShouldBeLoaded());
		LevelObj->SetBoolField(TEXT("should_be_visible"), LS->ShouldBeVisible());
		LevelObj->SetBoolField(TEXT("is_loaded"), LS->IsLevelLoaded());
		LevelObj->SetBoolField(TEXT("is_visible"), LS->IsLevelVisible());

		if (LS->IsA<ULevelStreamingDynamic>())
		{
			LevelObj->SetStringField(TEXT("streaming_method"), TEXT("Blueprint"));
		}
		else if (LS->IsA<ULevelStreamingAlwaysLoaded>())
		{
			LevelObj->SetStringField(TEXT("streaming_method"), TEXT("AlwaysLoaded"));
		}
		else
		{
			LevelObj->SetStringField(TEXT("streaming_method"), LS->GetClass()->GetName());
		}

		LevelArray.Add(MakeShared<FJsonValueObject>(LevelObj));
	}

	Result->SetArrayField(TEXT("streaming_levels"), LevelArray);
	Result->SetNumberField(TEXT("count"), LevelArray.Num());

	return CreateSuccessResponse(Result);
}

// ----------------------------------------------------------------------------
// FAddStreamingLevelAction
// ----------------------------------------------------------------------------

bool FAddStreamingLevelAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	FString LevelPath;
	return GetRequiredString(Params, TEXT("level_path"), LevelPath, OutError);
}

TSharedPtr<FJsonObject> FAddStreamingLevelAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	FString LevelPath, Error;
	GetRequiredString(Params, TEXT("level_path"), LevelPath, Error);
	...
	for (ULevelStreaming* LS : World->GetStreamingLevels())
	{
		if (LS && LS->GetWorldAssetPackageName() == LevelPath)
		{
			TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
	...
	// Mark the world package dirty so the change is saveable
	World->GetOutermost()->MarkPackageDirty();

	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
	Result->SetStringField(TEXT("level_path"), LevelPath);
	Result->SetBoolField(TEXT("already_exists"), false);
	Result->SetStringField(TEXT("class"), NewLevel->GetClass()->GetName());

	return CreateSuccessResponse(Result);
}

// ----------------------------------------------------------------------------
// FRemoveStreamingLevelAction
// ----------------------------------------------------------------------------

bool FRemoveStreamingLevelAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	FString LevelPath;
	return GetRequiredString(Params, TEXT("level_path"), LevelPath, OutError);
}

TSharedPtr<FJsonObject> FRemoveStreamingLevelAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	FString LevelPath, Error;
	GetRequiredString(Params, TEXT("level_path"), LevelPath, Error);
	...
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Streaming level not found: %s"), *LevelPath),
			TEXT("not_found"));
	}

	ULevel* LoadedLevel = Found->GetLoadedLevel();
	if (LoadedLevel)
	{
	...
	{
		World->RemoveStreamingLevel(Found);
	}
...
// ----------------------------------------------------------------------------
// FSetStreamingLevelMethodAction
// ----------------------------------------------------------------------------

bool FSetStreamingLevelMethodAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	FString LevelPath, Method;
	if (!GetRequiredString(Params, TEXT("level_path"), LevelPath, OutError)) return false;
	if (!GetRequiredString(Params, TEXT("streaming_method"), Method, OutError)) return false;
	if (Method != TEXT("Blueprint") && Method != TEXT("AlwaysLoaded"))
	{
		OutError = FString::Printf(TEXT("Invalid streaming_method '%s'. Use 'Blueprint' or 'AlwaysLoaded'."), *Method);
		return false;
	}
	return true;
}

TSharedPtr<FJsonObject> FSetStreamingLevelMethodAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	FString LevelPath, Method, Error;
	GetRequiredString(Params, TEXT("level_path"), LevelPath, Error);
	GetRequiredString(Params, TEXT("streaming_method"), Method, Error);
	...
	TSubclassOf<ULevelStreaming> NewClass = (Method == TEXT("Blueprint"))
		? ULevelStreamingDynamic::StaticClass()
		: ULevelStreamingAlwaysLoaded::StaticClass();

	ULevelStreaming* NewLS = UEditorLevelUtils::SetStreamingClassForLevel(Found, NewClass);

	World->GetOutermost()->MarkPackageDirty();

	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
	Result->SetStringField(TEXT("level_path"), LevelPath);
	Result->SetStringField(TEXT("streaming_method"), Method);
	Result->SetStringField(TEXT("new_class"), NewLS ? NewLS->GetClass()->GetName() : TEXT("unknown"));
...
// ----------------------------------------------------------------------------
// FMoveActorToLevelAction
// ----------------------------------------------------------------------------

bool FMoveActorToLevelAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	FString ActorName;
	return GetRequiredString(Params, TEXT("actor_name"), ActorName, OutError);
}

TSharedPtr<FJsonObject> FMoveActorToLevelAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	FString ActorName, Error;
	GetRequiredString(Params, TEXT("actor_name"), ActorName, Error);

	const FString DestLevelPath = GetOptionalString(Params, TEXT("dest_level"));
	...
	ULevel* DestLevel = nullptr;
	FString DestDescription;

	if (DestLevelPath.IsEmpty() || DestLevelPath.Equals(TEXT("persistent"), ESearchCase::IgnoreCase))
	{
		DestLevel = World->PersistentLevel;
		DestDescription = TEXT("PersistentLevel");
	}
	else
	{
		for (ULevelStreaming* LS : World->GetStreamingLevels())
		{
			if (LS && LS->GetWorldAssetPackageName() == DestLevelPath)
			{
				DestLevel = LS->GetLoadedLevel();
				DestDescription = DestLevelPath;
				break;
			}
		}
	}
	...
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Destination level not found or not loaded: %s"),
				DestLevelPath.IsEmpty() ? TEXT("persistent") : *DestLevelPath),
			TEXT("dest_not_found"));
	}

	if (Actor->GetLevel() == DestLevel)
	{
		TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
		Result->SetStringField(TEXT("actor_name"), ActorName);
		Result->SetStringField(TEXT("dest_level"), DestDescription);
		Result->SetBoolField(TEXT("already_in_dest"), true);
		return CreateSuccessResponse(Result);
	}
	...
	const int32 Moved = UEditorLevelUtils::MoveActorsToLevel(ActorsToMove, DestLevel, false, false);

	if (Moved == 0)
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Failed to move actor '%s' to level '%s'"), *ActorName, *DestDescription),
			TEXT("move_failed"));
	}
	...
	Result->SetStringField(TEXT("actor_name"), ActorName);
	Result->SetStringField(TEXT("dest_level"), DestDescription);
	Result->SetNumberField(TEXT("moved_count"), Moved);
	Result->SetBoolField(TEXT("already_in_dest"), false);
