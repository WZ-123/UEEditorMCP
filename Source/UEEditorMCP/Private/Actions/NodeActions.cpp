...
TSharedPtr<FJsonObject> FAddBlueprintGetSubsystemNodeAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	// UE 5.5: UK2Node_GetSubsystemFromPC and UK2Node_GetEngineSubsystem are not exported.
	// Use runtime UClass lookup + CreateNode with the exported base class UK2Node_GetSubsystem.
	if (FoundClass->IsChildOf(ULocalPlayerSubsystem::StaticClass()))
	{
		...
		if (NodeClass)
		{
			CreatedNode = FEdGraphSchemaAction_K2NewNode::CreateNode(
				TargetGraph, TArrayView<UEdGraphPin*>(), Position,
				[NodeClass](UEdGraph* InGraph) -> UK2Node* { return NewObject<UK2Node>(InGraph, NodeClass); },
				[FoundClass](UK2Node* NewNode) { CastChecked<UK2Node_GetSubsystem>(NewNode)->Initialize(FoundClass); },
	...
	{
		UClass* NodeClass = FindObject<UClass>(nullptr, TEXT("/Script/BlueprintGraph.K2Node_GetEngineSubsystem"));
		if (NodeClass)
		{
			CreatedNode = FEdGraphSchemaAction_K2NewNode::CreateNode(
				TargetGraph, TArrayView<UEdGraphPin*>(), Position,
...
TSharedPtr<FJsonObject> FCreateBlueprintFunctionAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	const bool bImplementParentOverride = GetOptionalBool(Params, TEXT("implement_parent_override"), false);
	...
	for (UEdGraph* Graph : Blueprint->FunctionGraphs)
	{
		if (Graph->GetFName() == FName(*FunctionName))
		{
			...
			for (UEdGraphNode* Node : Graph->Nodes)
			{
				...
				{
					ResultNodeId = ResultNode->NodeGuid.ToString();
				}
			}
			...
			if (!ResultNodeId.IsEmpty())
			{
				ResultData->SetStringField(TEXT("result_node_id"), ResultNodeId);
			}
			return CreateSuccessResponse(ResultData);
		}
	}
	...
	if (bImplementParentOverride)
	{
		UFunction* OverrideFunc = nullptr;
		UClass* const OverrideFuncClass = FBlueprintEditorUtils::GetOverrideFunctionClass(
			Blueprint, FName(*FunctionName), &OverrideFunc);
		if (!OverrideFuncClass || !OverrideFunc)
		{
			return CreateErrorResponse(FString::Printf(
				TEXT("Parent override function not found: %s"), *FunctionName));
		}

		FBlueprintEditorUtils::AddFunctionGraph<UClass>(Blueprint, NewGraph, false, OverrideFuncClass);
	}
	else
	{
	...
	// Add input parameters (skip when implementing a parent override — signature comes from parent)
	const TArray<TSharedPtr<FJsonValue>>* InputsArray = bImplementParentOverride ? nullptr : GetOptionalArray(Params, TEXT("inputs"));
	...
	const TArray<TSharedPtr<FJsonValue>>* OutputsArray = bImplementParentOverride ? nullptr : GetOptionalArray(Params, TEXT("outputs"));
	if (OutputsArray)
	{
		if (!ResultNode)
		{
	...
	if (!bImplementParentOverride && bIsPure && EntryNode)
	{
	...
	ResultData->SetBoolField(TEXT("implement_parent_override"), bImplementParentOverride);
...
TSharedPtr<FJsonObject> FAddSpawnActorFromClassNodeAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	// UK2Node_SpawnActorFromClass is not safe to instantiate through the generic
	// SpawnNode<T>() path in UE 5.5. That path calls PostPlacedNewNode() before
	// the node's default pins exist, but this node's PostPlacedNewNode()
	// immediately queries ScaleMethodPin. Create the node explicitly and ensure
	// pins exist before PostPlacedNewNode() runs.
	TargetGraph->Modify();
	...
	if (TargetGraph->HasAnyFlags(RF_Transactional))
	{
	...
	SpawnNode->NodePosX = FMath::RoundToInt(Position.X);
	SpawnNode->NodePosY = FMath::RoundToInt(Position.Y);
