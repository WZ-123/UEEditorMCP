...
// Niagara port (from Monolith plugin) — registry + handlers live in Private/Niagara/
#include "Niagara/MonolithToolRegistry.h"
#include "Niagara/MonolithJsonUtils.h"
#include "Niagara/MonolithNiagaraActions.h"
#include "Niagara/MonolithNiagaraLayoutActions.h"
...
void UMCPBridge::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
		UE_LOG(LogMCP, Log, TEXT("UEEditorMCP: Skipping TCP server startup while running commandlet"));
		return;
	}
...
void UMCPBridge::RegisterActions()
{
	...
	ActionHandlers.Add(TEXT("get_input_action"), MakeShared<FGetInputActionAction>());
	ActionHandlers.Add(TEXT("update_input_action"), MakeShared<FUpdateInputActionAction>());
	ActionHandlers.Add(TEXT("get_input_mapping_context"), MakeShared<FGetInputMappingContextAction>());
	...
	ActionHandlers.Add(TEXT("get_asset_sc_status"), MakeShared<FGetAssetSCStatusAction>());
	ActionHandlers.Add(TEXT("diff_blueprint_revisions"), MakeShared<FDiffBlueprintRevisionsAction>());
	...
	ActionHandlers.Add(TEXT("get_selected_actors"), MakeShared<FGetSelectedActorsAction>());
	ActionHandlers.Add(TEXT("get_outliner_tree"), MakeShared<FGetOutlinerTreeAction>());

	// =========================================================================
	// DataTable Actions
	// =========================================================================
	ActionHandlers.Add(TEXT("export_data_table_to_csv"), MakeShared<FExportDataTableToCsvAction>());
	ActionHandlers.Add(TEXT("import_data_table_from_csv"), MakeShared<FImportDataTableFromCsvAction>());
	...
	// =========================================================================
	ActionHandlers.Add(TEXT("get_streaming_levels"), MakeShared<FGetStreamingLevelsAction>());
	ActionHandlers.Add(TEXT("add_streaming_level"), MakeShared<FAddStreamingLevelAction>());
	ActionHandlers.Add(TEXT("remove_streaming_level"), MakeShared<FRemoveStreamingLevelAction>());
	ActionHandlers.Add(TEXT("set_streaming_level_method"), MakeShared<FSetStreamingLevelMethodAction>());
	ActionHandlers.Add(TEXT("move_actor_to_level"), MakeShared<FMoveActorToLevelAction>());

	// =========================================================================
	// Niagara Actions (ported from Monolith plugin)
	// All actions are exposed through the niagara_<action_name> command prefix.
	// e.g. type="niagara_add_emitter", type="niagara_get_system_summary"
	// =========================================================================
	{
		FMonolithToolRegistry& Registry = FMonolithToolRegistry::Get();
		FMonolithNiagaraActions::RegisterActions(Registry);
		FMonolithNiagaraLayoutActions::RegisterActions(Registry);
		UE_LOG(LogMCP, Log, TEXT("UEEditorMCP: Registered %d niagara actions (Monolith port)"),
			Registry.GetActions(TEXT("niagara")).Num());
	}
...
{
	// =========================================================================
	// Niagara dispatch (Monolith port) — type="niagara_<action_name>"
	// Routed to FMonolithToolRegistry; result/error converted to UEEditorMCP
	// JSON shape (status/success/result/error/error_type).
	// =========================================================================
	if (CommandType.StartsWith(TEXT("niagara_")))
	{
		const FString ActionName = CommandType.Mid(8);
		FMonolithActionResult R = FMonolithToolRegistry::Get().ExecuteAction(
			TEXT("niagara"), ActionName, Params);

		if (R.bSuccess)
		{
			return CreateSuccessResponse(R.Result);
		}
		const FString ErrType = (R.ErrorCode == FMonolithJsonUtils::ErrMethodNotFound)
			? TEXT("unknown_command")
			: TEXT("error");
		return CreateErrorResponse(R.ErrorMessage, ErrType);
	}
