...
namespace MonolithNiagaraHelpers
{
	...
	// =========================================================================
	// UE 5.5 compatibility shims for FNiagaraStackGraphUtilities helpers that
	// are declared in the public header but lack NIAGARAEDITOR_API export.
	// (Originally Monolith was written against UE 5.7 where these may have
	//  different APIs.) Cross-module call → unresolved external symbol at link
	//  time, so we re-implement minimal equivalents here.
	// =========================================================================
	void GetStackFunctionInputsCompat(
		const UNiagaraNodeFunctionCall& Node,
		TArray<FNiagaraVariable>& OutInputs,
		const FCompileConstantResolver& /*Resolver*/,
		FNiagaraStackGraphUtilities::ENiagaraGetStackFunctionInputPinsOptions /*Options*/,
		bool /*bIgnoreDisabled*/)
	{
		// Simplified equivalent: enumerate input pins, drop ParameterMap, build
		// FNiagaraVariable using the schema's type resolution. Accurate enough
		// for module-level read/write actions.
		OutInputs.Reset();
		const UEdGraphSchema_Niagara* Schema = GetDefault<UEdGraphSchema_Niagara>();
		for (const UEdGraphPin* Pin : Node.Pins)
		{
			...
			const FNiagaraTypeDefinition TypeDef = Schema->PinToTypeDefinition(Pin);
			if (!TypeDef.IsValid() || TypeDef == FNiagaraTypeDefinition::GetParameterMapDef())
			{
				continue;
			}
			OutInputs.Add(FNiagaraVariable(TypeDef, Pin->PinName));
		}
	}

	void GetStackFunctionStaticSwitchPinsCompat(
		const UNiagaraNodeFunctionCall& Node,
		...
		FCompileConstantResolver& /*Resolver*/)
	{
		...
		const TArray<FNiagaraVariable> SwitchInputs = CalledGraph->FindStaticSwitchInputs();
		for (const FNiagaraVariable& SwitchInput : SwitchInputs)
		{
			const FEdGraphPinType PinType = Schema->TypeDefinitionToPinType(SwitchInput.GetType());
			for (UEdGraphPin* Pin : Node.Pins)
			{
	...
	TSet<FNiagaraVariable> GetParametersForContextCompat(UEdGraph* /*InGraph*/, UNiagaraSystem& System)
	{
		// Engine's GetParametersForContext also walks UNiagaraGraph::GetParameterReferenceMap
		// (non-exported across modules), so we restrict ourselves to user parameters here —
		// this matches the original Monolith helper (collected from the exposed parameter
		// store). Sufficient for SetLinkedValueHandleForFunctionInput, which only needs the
		// known-parameter set to determine whether the handle is novel.
		TSet<FNiagaraVariable> Result;
		FNiagaraUserRedirectionParameterStore& UserStore = System.GetExposedParameters();
		TArrayView<const FNiagaraVariableWithOffset> Vars = UserStore.ReadParameterVariables();
		for (const FNiagaraVariableWithOffset& V : Vars)
		{
			Result.Add(V);
		}
...
FMonolithActionResult FMonolithNiagaraActions::HandleSetModuleInputBinding(const TSharedPtr<FJsonObject>& Params)
{
	...
	// UE 5.5 signature: SetLinkedValueHandleForFunctionInput(pin, FNiagaraParameterHandle, TSet<FNiagaraVariable>, ...)
	// (Monolith was written against 5.7's SetLinkedParameterValueForFunctionInput which takes
	//  a FNiagaraVariable and TSet<FNiagaraVariableBase>; in 5.5 we use the engine-provided
	//  GetParametersForContext that already returns TSet<FNiagaraVariable>.)
	TSet<FNiagaraVariable> KnownParams = Graph
		? MonolithNiagaraHelpers::GetParametersForContextCompat(Graph, *System)
		: TSet<FNiagaraVariable>();
	FNiagaraStackGraphUtilities::SetLinkedValueHandleForFunctionInput(
		OP,
		FNiagaraParameterHandle(LinkedParam.GetName()),
		KnownParams);
