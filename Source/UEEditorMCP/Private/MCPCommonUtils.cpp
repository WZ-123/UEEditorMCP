...
namespace MCPPropertyJson
{
static bool LoadClassFromPath(const FString& ClassPath, UClass*& OutClass, FString& OutErrorMessage)
{
	OutClass = LoadObject<UClass>(nullptr, *ClassPath);
	if (!OutClass)
	{
		const FString BlueprintPath = FString::Printf(TEXT("/Game/Blueprints/%s.%s_C"), *ClassPath, *ClassPath);
		OutClass = LoadObject<UClass>(nullptr, *BlueprintPath);
	}
...
static bool SetPropertyValueFromJson(FProperty* Property, void* PropertyAddr,
	const TSharedPtr<FJsonValue>& Value, FString& OutErrorMessage);

static bool SetStructFieldsFromJsonObject(UScriptStruct* Struct, void* StructAddr,
	const TSharedPtr<FJsonObject>& JsonObject, FString& OutErrorMessage)
{
	for (const TPair<FString, TSharedPtr<FJsonValue>>& FieldPair : JsonObject->Values)
	{
		FProperty* FieldProperty = Struct->FindPropertyByName(FName(*FieldPair.Key));
		if (!FieldProperty)
		{
			continue;
		}

		void* FieldAddr = FieldProperty->ContainerPtrToValuePtr<void>(StructAddr);
		if (!SetPropertyValueFromJson(FieldProperty, FieldAddr, FieldPair.Value, OutErrorMessage))
		{
			return false;
		}
	}
	...
	const TSharedPtr<FJsonValue>& Value, FString& OutErrorMessage)
{
	...
	{
		IntProp->SetIntPropertyValue(PropertyAddr, static_cast<int64>(Value->AsNumber()));
		return true;
	}
	...
	else if (FStructProperty* StructProp = CastField<FStructProperty>(Property))
	{
		...
		{
			return SetStructFieldsFromJsonObject(StructProp->Struct, PropertyAddr, Value->AsObject(), OutErrorMessage);
		}
	}
	else if (FClassProperty* ClassProp = CastField<FClassProperty>(Property))
	{
		if (Value->Type != EJson::String)
		{
			OutErrorMessage = TEXT("Class property expects a string path");
			return false;
		}

		UClass* LoadedClass = nullptr;
		if (!LoadClassFromPath(Value->AsString(), LoadedClass, OutErrorMessage))
		{
			return false;
		}
	...
	else if (FArrayProperty* ArrayProp = CastField<FArrayProperty>(Property))
	{
		if (Value->Type != EJson::Array)
		{
			OutErrorMessage = TEXT("Array property expects a JSON array");
			return false;
		}
		...
		for (const TSharedPtr<FJsonValue>& ElementValue : Elements)
		{
			...
			void* ElementAddr = ArrayHelper.GetRawPtr(NewIndex);
			if (!SetPropertyValueFromJson(ArrayProp->Inner, ElementAddr, ElementValue, OutErrorMessage))
			{
				return false;
			}
		}
	...
	OutErrorMessage = FString::Printf(TEXT("Unsupported property type: %s"), *Property->GetName());
	return false;
}
} // namespace MCPPropertyJson
	...
	const TSharedPtr<FJsonValue>& Value, FString& OutErrorMessage)
{
	...
	return MCPPropertyJson::SetPropertyValueFromJson(Property, PropertyAddr, Value, OutErrorMessage);
}
