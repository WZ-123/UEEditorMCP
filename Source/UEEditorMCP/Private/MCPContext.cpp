...
void FMCPEditorContext::SaveDirtyPackages()
{
	...
	for (UPackage* Package : DirtyPackages)
	{
		...
		// UPackage::SavePackage internally falls back to FindAssetInPackage when
		// AssetToSave is null, but providing it explicitly avoids edge cases where
		// the internal fallback may trigger assertions (e.g. __debugbreak()) in
		// certain UE 5.5 configurations when the asset is in an invalid state.
		UObject* AssetToSave = Package->FindAssetInPackage();
		if (!AssetToSave)
		{
			UE_LOG(LogMCP, Warning, TEXT("UEEditorMCP: SaveDirtyPackages - No asset found in package '%s' (isMap=%d), attempting save with nullptr"), *PackageName, bIsMap ? 1 : 0);
		}

		UE_LOG(LogMCP, Log, TEXT("UEEditorMCP: SaveDirtyPackages - Saving package '%s' -> '%s' (asset=%s)"),
		...
		{
			SavedCount++;
			UE_LOG(LogMCP, Log, TEXT("UEEditorMCP: SaveDirtyPackages - Successfully saved '%s'"), *PackageName);
		}
		else
		{
