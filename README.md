...
一个面向 AI 辅助 Unreal Editor 开发的 MCP 插件，覆盖蓝图、材质、UMG、编辑器、日志和性能分析等工作流。  
对外暴露 **7 个固定 MCP 工具**，由后端 **动作注册表（Action Registry）** 驱动，动作能力可持续扩展，同时提供持久 TCP 连接、多客户端支持与自动保存。
...
- **动作注册表** — 蓝图、图节点、材质、UMG 控件、MVVM、增强输入、组件事件绑定、PIE 启停/状态查询、日志断言、Outliner 管理、资产缩略图提取、批量资产重命名及重定向修复、与版本控制仓库的差异对比 — 均可通过动作注册表动态发现
- **蓝图 / 材质操作稳定性增强** — 在 lilklon/UEBlueprintMCP 基础上增强蓝图与材质编辑动作的校验、诊断和执行稳定性
...
VS Code / MCP 客户端（GitHub Copilot 等）
  ...
  C++ 插件（FMCPServer → 每连接一个 FMCPClientHandler）
        ...
        │  游戏线程分发
        ▼
  FEditorAction / Niagara Action / Editor API → 校验 → 执行 → 自动保存
        │
        ▼
  Unreal Editor
```
...
将 `Engine\Build\BatchFiles` 换成本机 UE 安装路径，例如 `F:\UE_5.x\Engine\Build\BatchFiles\Build.bat`；`YourProjectEditor` 和 `.uproject` 路径也换成你的项目名。也可以直接使用 Visual Studio / Rider / VS Code 任务编译编辑器目标。

### 第 2 步：一键 Python 环境配置（使用 UE 引擎内置 Python）

**通常无需安装外部 Python。** 配置脚本会自动找到 Unreal Engine 内置 Python，创建虚拟环境并安装 MCP 包。
...
5. 在项目根目录生成 `.vscode/mcp.json` 和 `.cursor/mcp.json`，并填入正确路径
...
然后在项目根目录创建 `.vscode/mcp.json`。手写配置时推荐使用绝对路径；不同 MCP 客户端对相对路径的解析基准可能不同，自动脚本生成的配置最可靠。
...
- `assetPath`（必填）：完整资产路径（如 `/Game/td_digging/Blueprints/BP_Foo`）
...
| `Python/ue_editor_mcp/registry/actions.py` | 带模式/标签/示例的 ActionDef 注册表 |
...
| `Source/UEEditorMCP/Private/MCPServer.cpp` | TCP Accept 循环 + 每客户端处理线程 |
| `Source/UEEditorMCP/Private/MCPBridge.cpp` | C++ 命令注册与分发中心 |
| `Source/UEEditorMCP/Private/Actions/*.cpp` | `FEditorAction` 子类具体实现 |
...
7 工具统一 MCP 服务器、动作注册表、关键字搜索、批量执行、嵌入式文档和结构化日志——**全部完成**。
