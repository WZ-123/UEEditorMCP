"""
Action definitions — all 116 C++ actions registered with metadata.
...
_EDITOR_ACTIONS = [
    ...
    ActionDef(
        ...
        tags=("editor", "asset", "duplicate", "copy", "clone"),
        description=(
            "Duplicate an asset in the Content Browser. "
            "The new asset is immediately registered with the Asset Registry — no editor restart needed. "
            "Useful for creating temporary copies before merging Blueprint changes."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "asset_path": {
                    "type": "string",
                    "description": "Full path of the source asset (e.g. /Game/Actors/BP_Hero)",
                },
                "new_name": {
                    "type": "string",
                    "description": "Name for the duplicate (no path prefix)",
                },
                "new_package_path": {
                    "type": "string",
                    "description": "Destination folder (default: same folder as source)",
                },
            },
        ...
        risk="moderate",
        examples=(
            {"asset_path": "/Game/Actors/BP_Hero", "new_name": "BP_Hero_Backup"},
            {"asset_path": "/Game/Actors/BP_Hero", "new_name": "BP_Hero_Copy", "new_package_path": "/Game/Temp"},
        ),
    ),
    ActionDef(
        id="editor.delete_asset",
        command="delete_asset",
        tags=("editor", "asset", "delete", "remove", "cleanup"),
        description=(
            "Delete an asset from the project. Checks source-control status first: "
            "if the asset has uncommitted SC modifications, returns an error unless force=true. "
            "If SC is enabled and the asset is tracked, also marks it for delete in source control."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "asset_path": {
                    "type": "string",
                    "description": "Full path of the asset to delete (e.g. /Game/Temp/BP_TempCopy)",
                },
                "force": {
                    "type": "boolean",
                    "description": "Skip SC safety checks and force delete (default: false)",
                },
            },
            "required": ["asset_path"],
        },
        risk="destructive",
        examples=(
            {"asset_path": "/Game/Temp/BP_TempCopy"},
            {"asset_path": "/Game/Actors/BP_Old", "force": True},
        ),
    ),
    ActionDef(
        id="editor.get_asset_sc_status",
        command="get_asset_sc_status",
        tags=("editor", "asset", "source-control", "status", "git", "merge", "base", "revision", "read"),
        description=(
            "Get detailed source-control status for an asset. Returns: "
            "SC flags (modified, checked-out, added, deleted, conflicted, current), "
            "localBaseRevision (last commit on current branch that touched this file — the user's true base), "
            "remoteLatestRevision (last commit on remote tracking branch — the latest pushed version), "
            "remoteHasNewerVersion (true if remote has commits the user hasn't pulled), "
            "isPackageDirty (unsaved in-memory changes). "
            "Use this to auto-detect the merge base without asking the user for a commit hash."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "asset_path": {
                    "type": "string",
                    "description": "Full asset path (e.g. /Game/Actors/BP_Hero)",
                },
            },
            "required": ["asset_path"],
        },
        ...
        risk="safe",
        examples=(
            {"asset_path": "/Game/Actors/ALS_AnimMan_CharacterBP_BearMan"},
        ),
    ),
    ActionDef(
        id="blueprint.diff_revisions",
        command="diff_blueprint_revisions",
        tags=("blueprint", "diff", "revision", "compare", "merge", "source-control", "history", "read"),
        description=(
            "Diff a Blueprint between two source-control revisions (by commit hash or revision string). "
            "Supports short hash prefix matching. If revision_b is omitted, diffs revision_a against "
            "the current local working copy — useful for seeing what changed since a specific commit. "
            "Returns per-graph node-level changes identical to editor.diff_against_depot."
        ),
        input_schema={
            "type": "object",
            "properties": {
                ...
                "revision_a": {
                    "type": "string",
                    "description": "Older revision (commit hash or revision string, supports short prefix)",
                },
                "revision_b": {
                    "type": "string",
                    "description": "Newer revision (omit to diff against local working copy)",
                },
            },
            "required": ["asset_path", "revision_a"],
        },
        capabilities=("read",),
        risk="safe",
        examples=(
            {"asset_path": "/Game/Actors/BP_Hero", "revision_a": "184793c5"},
            {"asset_path": "/Game/Actors/BP_Hero", "revision_a": "184793c5", "revision_b": "a1b2c3d4"},
        ),
    ),
    ActionDef(
        id="editor.get_logs",
    ...
    ActionDef(
        id="editor.get_selected_actors",
        command="get_selected_actors",
        tags=("editor", "actors", "selected", "selection", "viewport", "read"),
        description="Get the currently selected actors in the level viewport.",
        input_schema={"type": "object", "properties": {}},
        capabilities=("read",),
        risk="safe",
    ),
    ...
    # =========================================================================
    ActionDef(
        id="editor.export_data_table_to_csv",
        command="export_data_table_to_csv",
        tags=("editor", "datatable", "csv", "export"),
        description=(
            "Export a DataTable asset to a CSV file on disk. "
            "Use the returned row_struct path when re-importing."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "data_table_path": {
                    "type": "string",
                    "description": "Content path, e.g. '/DestructibleCaveGenerator/.../DT_Demo'",
                },
                "csv_path": {
                    "type": "string",
                    "description": "Output .csv path (absolute or relative to project directory)",
                },
            },
            "required": ["data_table_path", "csv_path"],
        },
        capabilities=("read",),
        risk="safe",
        examples=(
            {
                "data_table_path": "/DestructibleCaveGenerator/DestructibleCaveGenerator/DataTables/DT_Demo",
                "csv_path": "Content/DestructibleCaveGenerator/DataTables/DT_Demo.csv",
            },
        ),
    ),
    ActionDef(
        id="editor.import_data_table_from_csv",
        command="import_data_table_from_csv",
        tags=("editor", "datatable", "csv", "import", "terrain"),
        description=(
            "Import a CSV into an existing DataTable using an explicit row struct path "
            "(no manual struct picker). For DT_Demo terrain coins use "
            "row_struct_path='/Script/DestructibleCaveGenerator.TerrainData'."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "data_table_path": {
                    "type": "string",
                    "description": "DataTable to overwrite, e.g. '/DestructibleCaveGenerator/.../DT_Demo'",
                },
                "csv_path": {
                    "type": "string",
                    "description": "Source .csv path (absolute or relative to project directory)",
                },
                "row_struct_path": {
                    "type": "string",
                    "description": "Row struct object path, e.g. '/Script/DestructibleCaveGenerator.TerrainData'",
                },
                "preserve_existing_values": {
                    "type": "boolean",
                    "description": "If true, keep existing cell values when CSV cell is empty (default false)",
                },
                "try_asset_import_fallback": {
                    "type": "boolean",
                    "description": "If fill import fails, retry via AssetImportTask (default true)",
                },
                "verify_row_name": {
                    "type": "string",
                    "description": "Optional row name to spot-check after import (e.g. 'Grass')",
                },
                "verify_substring": {
                    "type": "string",
                    "description": "Optional substring expected in that row's CSV line (e.g. '0.400000')",
                },
            },
            "required": ["data_table_path", "csv_path", "row_struct_path"],
        },
        capabilities=("write",),
        risk="moderate",
        examples=(
            {
                ...
                "csv_path": "Content/DestructibleCaveGenerator/DataTables/DT_Demo.csv",
                "row_struct_path": "/Script/DestructibleCaveGenerator.TerrainData",
                "verify_row_name": "Grass",
                "verify_substring": "0.400000",
            },
            {
                "data_table_path": "/DestructibleCaveGenerator/DestructibleCaveGenerator/DataTables/DT_Demo",
                "csv_path": "Plugins/Destruct9c1d5b5fb571V1/Content/DestructibleCaveGenerator/DataTables/DT_Demo.csv",
                "row_struct_path": "/Script/DestructibleCaveGenerator.TerrainData",
            },
        ),
    ),
    ...
    ActionDef(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "implement_parent_override": {
                    "type": "boolean",
                    "description": "If true, create a function graph that overrides a parent/C++ BlueprintImplementableEvent (uses parent signature; do not pass inputs/outputs)"
                }
            },
            "required": ["blueprint_name", "function_name"]
        },
...
_INPUT_ACTIONS = [
    ...
    ActionDef(
        ...
        tags=("input", "action", "enhanced", "read", "inspect"),
        description="Read an Enhanced Input Action asset's editable properties",
        input_schema={
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "Name of the Input Action asset"},
                "path": {"type": "string", "description": "Content path containing the asset (default: /Game/Input)"},
                "asset_path": {"type": "string", "description": "Optional full asset path like /Game/Input/IA_Move or /Game/Input/IA_Move.IA_Move"}
            }
        },
        examples=(
            {"name": "IA_Move", "path": "/Game/Game/EnhancedInput"},
            {"asset_path": "/Game/Game/EnhancedInput/IA_Move"},
        ),
    ),
    ActionDef(
        id="input.update_action",
        command="update_input_action",
        tags=("input", "action", "enhanced", "update", "property"),
        description="Update editable properties on an existing Enhanced Input Action asset",
        input_schema={
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "Name of the Input Action asset"},
                ...
                "asset_path": {"type": "string", "description": "Optional full asset path like /Game/Input/IA_Move or /Game/Input/IA_Move.IA_Move"},
                "value_type": {"type": "string", "description": "Boolean, Axis1D/Float, Axis2D/Vector2D, Axis3D/Vector"},
                "accumulation_behavior": {"type": "string", "description": "TakeHighestAbsoluteValue or Cumulative"},
                "consume_input": {"type": "boolean", "description": "Set bConsumeInput"},
                "consume_legacy_mappings": {"type": "boolean", "description": "Set bConsumesActionAndAxisMappings"},
                "reserve_all_mappings": {"type": "boolean", "description": "Set bReserveAllMappings"},
                "trigger_when_paused": {"type": "boolean", "description": "Set bTriggerWhenPaused"},
                "trigger_events_that_consume_legacy_keys": {"type": "integer", "description": "Bitmask for ETriggerEvent when consuming legacy mappings"},
                "description": {"type": "string", "description": "ActionDescription text"}
            }
        },
        examples=(
            {"name": "IA_Move", "path": "/Game/Game/EnhancedInput", "value_type": "Axis2D", "accumulation_behavior": "Cumulative"},
        ),
    ),
    ActionDef(
        id="input.get_mapping_context",
        command="get_input_mapping_context",
        tags=("input", "mapping", "context", "enhanced", "read", "inspect"),
        description="Read an Enhanced Input Mapping Context asset's mappings, modifiers, and triggers",
        input_schema={
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "Name of the Input Mapping Context asset"},
                "path": {"type": "string", "description": "Content path containing the asset (default: /Game/Input)"},
                "asset_path": {"type": "string", "description": "Optional full asset path like /Game/Input/IMC_Default or /Game/Input/IMC_Default.IMC_Default"}
            }
        },
        examples=(
            {"name": "IMC_DigDefault", "path": "/Game/Game/EnhancedInput"},
            {"asset_path": "/Game/Game/EnhancedInput/IMC_DigDefault"},
        ),
    ),
    ActionDef(
    ...
    # =========================================================================
    # Level Streaming Management Actions
    # =========================================================================
    ActionDef(
        id="editor.get_streaming_levels",
        command="get_streaming_levels",
        tags=("editor", "level", "streaming", "read"),
        description="List all streaming sub-levels in the current persistent level with their loaded/visible state and streaming method",
        input_schema={"type": "object", "properties": {}},
        capabilities=("read",),
    ),
    ActionDef(
        id="editor.add_streaming_level",
        command="add_streaming_level",
        tags=("editor", "level", "streaming", "add"),
        description="Add a level as a streaming sub-level to the current persistent level",
        input_schema={
            "type": "object",
            "properties": {
                "level_path": {"type": "string", "description": "Package path of the level, e.g. /Game/Game/Levels/G_NYCity"},
                "streaming_method": {"type": "string", "enum": ["Blueprint", "AlwaysLoaded"], "description": "Streaming method (default: Blueprint)"}
            },
            "required": ["level_path"]
        },
        examples=({"level_path": "/Game/Game/Levels/G_NYCity"},),
    ),
    ActionDef(
        id="editor.remove_streaming_level",
        command="remove_streaming_level",
        tags=("editor", "level", "streaming", "remove", "delete"),
        description="Remove a streaming sub-level from the current persistent level",
        input_schema={
            "type": "object",
            "properties": {
                "level_path": {"type": "string", "description": "Package path of the level to remove"}
            },
            "required": ["level_path"]
        },
        examples=({"level_path": "/Game/Game/Levels/UG_CarPark"},),
    ),
    ActionDef(
        id="editor.set_streaming_level_method",
        command="set_streaming_level_method",
        tags=("editor", "level", "streaming", "method", "set"),
        description="Change the streaming method (Blueprint or Volume) on an existing streaming sub-level",
        input_schema={
            "type": "object",
            "properties": {
                "level_path": {"type": "string", "description": "Package path of the level"},
                "streaming_method": {"type": "string", "enum": ["Blueprint", "AlwaysLoaded"], "description": "New streaming method"}
            },
            "required": ["level_path", "streaming_method"]
        },
    ),
    ActionDef(
        id="editor.move_actor_to_level",
        command="move_actor_to_level",
        tags=("editor", "actor", "level", "move"),
        description="Move an actor to a different level. Use dest_level='persistent' or omit it to move to the persistent level, or provide a streaming sub-level package path.",
        input_schema={
            "type": "object",
            "properties": {
                "actor_name": {"type": "string", "description": "Name of the actor to move"},
                "dest_level": {"type": "string", "description": "Destination: 'persistent' (default) or a streaming level package path like /Game/Game/Levels/G_NYCity"}
            },
            "required": ["actor_name"]
        },
        examples=(
            {"actor_name": "TerrainManager_0", "dest_level": "persistent"},
            {"actor_name": "MyActor", "dest_level": "/Game/Game/Levels/UG_CarPark"},
        ),
    ),
]
