...
TOOLS = [
    ...
    Tool(
        name="unreal_asset_history_get",
        description="List source-control revision history for a given asset. Returns an array of revisions that actually modified the file (up to ~100 for SVN). Use this to discover valid revision numbers before calling unreal_asset_diff_get. Requires UE Editor running with Source Control connected.",
        inputSchema={
            "type": "object",
            "properties": {
                "assetPath": {
    ...
    Tool(
        name="unreal_asset_thumbnail_get",
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent | ImageContent]:
    ...
    # Backward aliases (older dotted names)
    _aliases = {
        "unreal.logs.get": "unreal_logs_get",
        "unreal.asset_thumbnail.get": "unreal_asset_thumbnail_get",
        "unreal.asset_diff.get": "unreal_asset_diff_get",
        "unreal.asset_history.get": "unreal_asset_history_get",
    }
    resolved = _aliases.get(name, name)
    if resolved not in _known:
        text = json.dumps({"success": False, "error": f"Unknown tool: {name}"}, ensure_ascii=False)
        return [TextContent(type="text", text=text)]

    try:
        if resolved == "unreal_logs_get":
            result = _handle_unreal_logs_get(arguments or {})
        elif resolved == "unreal_asset_diff_get":
            result = _handle_asset_diff_get(arguments or {})
        elif resolved == "unreal_asset_history_get":
    ...
    if resolved == "unreal_asset_thumbnail_get" and result.get("success"):
